...
namespace dagbase
{
    ...
    _lua(lua.get())
    {
        int errod = lua_load(lua.get(), &reader, (void*)&str, "chunk", "bt");
        if (errod!=LUA_OK)
        {
    ...
    {
        auto str = static_cast<InputStream*>(userData);
        ...
        if (str && size)
        {
            ...
            {
                *size = blockSize;
                auto buf = new InputStream::value_type [blockSize];
                str->readBuf(buf, *size);
